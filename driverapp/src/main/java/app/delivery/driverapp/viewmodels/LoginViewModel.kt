...
class LoginViewModel @Inject constructor(
        ...
        firebaseAuth.signInWithCredential(credential)
            .addOnCompleteListener(activity) { task ->
                if (task.isSuccessful) {
                    ...
                    val exception = task.exception ?: Exception("Verification failed")
                    Timber.e(exception)
                    FirebaseCrashlytics.getInstance().recordException(exception)
                    _viewState.value =
                        _viewState.value.copy(
    ...
    private fun performPostLogin() {
        viewModelScope.launch {
            ...
            userLoginUseCase.performPostLogin(name).collect { userResult ->
                userResult.fold(onSuccess = {
                    _viewState.value =
                        _viewState.value.copy(verifyingOTP = false, userCreated = true)
                }, onFailure = { throwable ->
                    _viewState.value = _viewState.value.copy(
                        verifyingOTP = false,
                        actionError = ViewEvent(throwable)
                    )
                })
            }
        }
